...
mydata %>% 
  ...
  select(starts_with("radius"), starts_with("texture"), starts_with("perimeter"), starts_with("area"), starts_with("smooth"), starts_with("compact"), starts_with("concavity"), starts_with("concave"), starts_with("symmetry"), starts_with("fractal")) %>% 
  select(-starts_with("area"), -starts_with("perimeter")) %>% 
  cor() %>% 
  ggcorrplot()
...
y <- as.vector(ifelse(mydata[,2] == "M", 1, 0))
newdat <- cbind.data.frame(y,X)
logfit <- glm(y~., family = binomial(link = "logit"), data = newdat)
```
...
cvresult <- do.call(cbind,lapply(list,function(x){
  ...
  for (i in 1:K){
    ...
    cv.errors[i] = mean((as.vector(y[folds==i])-as.vector(pred))^2)  # MSE
  }
...
text(0.3, 0.8, labels = paste("best lambda = ", round(best.lambda, 3), sep = ""))
