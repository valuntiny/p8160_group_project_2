...
coordinatelasso <- function(lambda, dat, start, tol=1e-10, maxiter = 200) {
  ...
  # weighted updates
  w <- prob*(1-prob)
  z <- u + ifelse(is.nan((dat$y-prob)/w),0,(dat$y-prob)/w) # some value with w=0, NaN
  # quardratic approximation to the log likelihood difference is constant(beta), had to be convex
  loglik <- 1/2*(1/n)*sum(w*(z-u)^2) + sum(lambda * abs(betavec[-pp])) # doesnot penalty intercept
  ...
  while(i < maxiter && abs(loglik - prevloglik) > tol && loglik > -Inf) {
    ...
    for (j in 1:pp){
      ...
      # weighted updates
      w <- prob*(1-prob)
      z <- u + ifelse(is.nan((dat$y-prob)/w),0,(dat$y-prob)/w)
    ...
    loglik <- 1/2*(1/n)*sum(w*(z-u)^2) + sum(lambda * abs(betavec[-pp]))
    res <- rbind(res, c(i, loglik, betavec))}  
  return(res)
}
corres <- coordinatelasso(lambda = exp(4), newdat, start = rep(0, dim(newdat$X)[2]),maxiter = 200)
# largest lambda: 60
check <- tail(corres)[,1:2]
cor.beta <- corres[nrow(corres),3:dim(corres)[2]]
library(glmnet)
# logmod <- glmnet(newdat$X[,-dim(newdat$X)[2]], y=newdat$y, alpha=1, family="binomial",lambda = 0.01)
```
...
path.out <- path(newX,resp,grid=exp(seq(5,-5, length=100)))
...
result <- cvresult(newX,resp,grid=exp(seq(5,-5, length=100)),K=5)
