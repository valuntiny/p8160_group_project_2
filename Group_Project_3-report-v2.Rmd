...
abstract: "This report discusses a study of bootstrap to develop classification model based on the proteins expression levels. Our goal is to build a predictive model based on logistic regression to facilicate Down syndrome diagnosis. We compared Pathwise Coordinate Descent Estimation with Regularized Logistic Regression (PCD-LASSO) and Smoothed Bootstrap Estimation (SBE). Our result shows that while the Cross Validation MSE are similar between these two methods, SBE provides a more accurate classification result And based on that, we identified a subset of proteins that are significantly associated with the Down syndrome."
thanks:
...
### Pathwise Coordinate Descent with regularized logistic regression (PCD-LASSO)
...
\textbf{Figure 3: results of PCD-LASSO}
...
\textbf{Figure 4: Lambda selection based on SBE for Logistic LASSO}
