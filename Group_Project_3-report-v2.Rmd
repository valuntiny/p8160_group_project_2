...
\textbf{Figure 1: missingness plot and multicollinearity plot of the dataset}
...
\textbf{Figure 2: Estimation path and cross validation for Pathwise Coordinate Descent Lasso}
...
\textbf{Figure 3: Lambda selection based on Smoothed Bootstrap for Logistic Lasso}
...
s1 <- ggplot(out.df,aes(x=out.df$subject.1)) + 
  ...
  #geom_density()
  ggtitle("Figure 1: Expected value of subject 1 from smoothed bootstrap")+
  theme(plot.title = element_text(size = 8))+
...
s2 <- ggplot(out.df,aes(x=out.df$min.lambda)) +
  geom_histogram(colour="black", fill="white")+
  #geom_density()
  ggtitle("Figure 1: penalty term from smoothed bootstrap")+
  theme(plot.title = element_text(size = 8))+
...
grid.arrange(s1+
  annotation_custom(grid.text(paste("Expected value from data= ", round(sub1, 4), sep = ""),
                              x=0.5,y=0.5, hjust=0,
                   gp=gpar(col="firebrick",fontsize=6, fontface="italic")))+
