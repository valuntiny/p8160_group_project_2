...
\textbf{Figure 1: proteins' missing percentage}
...
\textbf{Figure 2: missingness plot and multicollinearity plot of the dataset}
...
\textbf{Figure 3: Pathwise Coordinate Descent Lasso}
...
\textbf{Fig. 3A} shows us that as the $\lambda$ increases, all the variable estimates of parameters shrink accordingly since we penalize all the parameters, though in some. When $\lambda = 0$, the result is the same as least square method and when $\lambda$ is too large, all the estimates of parameters shrink to 0. \textbf{Fig. 3B} shows us the cross validation result for choosing the best $\lambda$. 
...
- do cross validation and select the best $\lambda$
- suggest $\lambda = \frac{1}{B}\sum_{i=1}^B \lambda_i^{(b)}$  bagging?

\textbf{Figure 4: Lambda selection based on Smoothed Bootstrap for Logistic Lasso}
...
We used 10 fold cross-validation to compare two different models, one is with $\lambda$ selected from data, the other is selected from the bootstrap.
...
for question 4, two method:
1-select covariates based on number of chosen, 
2-do hypothesis testing, select significant covariates.
...
kable(est.result[c(1:35),], "latex", caption = "Smoothed bootstrap estimation for best model(1)", booktabs = T) %>%
  kable_styling(latex_options = c("hold_position", "scale_down"),
                font_size = 6,full_width = F) %>%
  add_footnote(c("Bootstap time=5000"),
               notation = "alphabet")

kable(est.result[-c(1:35),], "latex", caption = "Smoothed bootstrap estimation for best model(2)", booktabs = T) %>%
  kable_styling(latex_options = c("hold_position", "scale_down"),
                font_size = 6,full_width = F) %>%
  add_footnote(c("Bootstap time=5000"),
               notation = "alphabet")
...
Chosen probability greater than 80\% will be considered as important;
Chosen probability greater than 96\% will be considered as significant(two exceptions);
Chosen probability greater than 96\% and confidence interval without zero might be a good choice.
