...
attach(oridata)
mydata <- oridata%>%
  select(.,-c(BAD_N,BCL2_N,H3AcK18_N,EGR1_N,H3MeK4_N))%>%
...
\[
\tilde{sd}_B = [\sum_{i=1}^n\hat{cov_j^2}]^{1/2}
\]
where 
\[
cov_j = \sum_{i=1}^B(Y_{ij}^*-Y_{.j}^*)(t_i^*-t_.^*)/B
\]
...
path.out <- path(dat,grid=exp(seq(-1,-10, length=10))) #100
...
result <- cvresult(dat,grid=exp(seq(-1,-10, length=10)),K=5) #100
...
finlasso <- as.matrix(path(dat,grid = exp(seq(-1,log(best.lambda), length=10)))) #100
...
taskFun <- function(){
  ...
  min <- which.min(unlist(lapply(1:4, function(x)mse(model[[x]]))))
  model.chosen <- min
...
subject.1 <- unlist(lapply(out$subject,mean))
